...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children, supabaseClient }) => {
  ...
  }, [user, profileFetchedForUserId, loading, isFetchingProfile, supabaseClient]);
  ...
  const forgotPassword = async (email: string) => {
    console.log("AuthContext: forgotPassword function called. Email:", email);
    setLoading(true);
    
    // Construct the redirect URL
    const redirectUrl = new URL('/reset-password', window.location.origin);
    
    console.log("Forgot password redirect URL:", redirectUrl.toString());
  ...
  const resetPassword = async (token: string, password: string) => {
    console.log("AuthContext: resetPassword function called.");
    setLoading(true);
    try {
      // Use the token to reset the password
      const { error } = await supabaseClient.auth.updateUser({ password });
      if (error) {
        console.error("AuthContext: Supabase resetPassword error:", error);
        throw error;
      }
  ...
  // NEW: Update last accessed module function
  const updateLastAccessedModule = async (moduleId: string) => {
    console.log("AuthContext: updateLastAccessedModule function called. Module ID:", moduleId);
    if (!user?.id) {
      console.error("AuthContext: Cannot update last accessed module - no user ID");
      return;
    }
    ...
    try {
      const { error } = await supabaseClient
        .from('users')
        .update({ last_accessed_module: moduleId })
        .eq('id', user.id);

      if (error) {
        console.error("AuthContext: Supabase updateLastAccessedModule error:", error);
        throw error;
      }
      
      // Update local user state
      setUser(prev => prev ? { ...prev, last_accessed_module: moduleId } : null);
      console.log("AuthContext: Last accessed module updated successfully.");
    } finally {
      setLoading(false);
    }
  };
