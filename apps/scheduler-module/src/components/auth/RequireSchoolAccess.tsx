...
const RequireSchoolAccess: React.FC<RequireSchoolAccessProps> = ({ children }) => {
  ...
  useEffect(() => {
    const checkSchoolAccess = async () => {
      ...
      if (!schoolSlug) {
        console.log('[RequireSchoolAccess] No schoolSlug, redirecting to school-select');
      ...
      if (!resolvedSchoolId) {
        ...
        if (!schoolId) {
          console.log('[RequireSchoolAccess] Failed to resolve school slug, redirecting to school-select');
          navigate('/school-select');
          return;
        }
