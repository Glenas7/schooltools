...
import { Building2, Users, Crown, Shield, Trash2, Loader2, Calendar, UtensilsCrossed, Settings, ChevronLeft, ChevronRight } from 'lucide-react'

interface SchoolSelectorProps {
  onSchoolSelect?: (schoolId: string | null) => void
  selectedSchoolId?: string | null // Add this prop to sync with parent
  showAllSchools?: boolean // Whether to show all schools or just selected
  onToggleShowAll?: () => void // Function to toggle show all state
}
...
const SchoolSelector: React.FC<SchoolSelectorProps> = ({ 
  ...
  const handleAdminClick = (e: React.MouseEvent, schoolSlug: string) => {
    e.stopPropagation()
    navigate(`/school/${schoolSlug}/manage`)
  }
          ...
          {school.modules && school.modules.length > 0 && (
            <div className="mb-3">
              ...
              <div className="flex flex-wrap gap-1">
                {school.modules.map((module, moduleIndex) => (
                  <div
                    key={`module-${module.module_id}-${moduleIndex}`}
          ...
          <div className="flex justify-between items-center">
            ...
            {(school.userRole === 'admin' || school.userRole === 'superadmin') && (
              <Button
                variant="outline"
                size="sm"
                onClick={(e) => handleAdminClick(e, school.slug)}
  ...
  const displayedSchools = showAllSchools 
    ? schools 
    : schools.filter(school => school.id === selectedSchoolId)

  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {displayedSchools.map((school, schoolIndex) => (
          <Card 
            key={`school-card-${school.id}-${schoolIndex}`}
              ...
              {school.modules && school.modules.length > 0 && (
                <div className="mb-3">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {school.modules.map((module, moduleIndex) => (
                      <div
                        key={`popup-module-${module.module_id}-${moduleIndex}`}
                        className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${getRoleColor(module.user_role)}`}
                      >
                        {getModuleIcon(module.module_name)}
                        <span className="capitalize">{module.module_display_name}</span>
        ...
        {/* Chevron Toggle Card */}
        {onToggleShowAll && schools.length > 1 && !showAllSchools && (
          <div className="flex items-center justify-left">
            <Card 
              className="cursor-pointer transition-all hover:shadow-lg border-2 border-gray-300 bg-white hover:bg-gray-50 flex items-center justify-center w-16 h-full"
